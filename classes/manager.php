...
namespace mod_reactive;

use context_module;
use cm_info;
use stdClass;
use mod_reactive\event\course_module_viewed;
...
/**
 * Class manager for reactive activity
 *
 * @package    mod_reactive
 * @since      Moodle 3.9
...
class manager {
    ...
    /** The plugin name. */
    public const PLUGIN = 'mod_reactive';

    /** The folder name. */
    public const FOLDER = 'reactive';
    ...
    public function put_entry($data) {
        ...
        if (empty($data->id)) {
            ...
            $record->id = $DB->insert_record('reactive_entries', $record);
            return $record;
        }
        ...
        if (!$DB->update_record('reactive_entries', $record)) {
    ...
    public function get_state(): stdClass {
        ...
        $entries = [];
        $records = $DB->get_records('reactive_entries', ['reactiveid' => $this->instance->id]);
